...
function rephraseConversation(text, tone) {
  const lines = String(text).split(/\r?\n/);
  const realLines = lines.filter((line) => cleanInput(line));
  if (realLines.length <= 1) {
    return rephrase(text, tone);
  }
  return lines
    .map((line) => (cleanInput(line) ? rephrase(line, tone) : ''))
    .join('\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
}
...
function updateOutput() {
  ...
  output.textContent = rephraseConversation(messageInput.value, selectedTone);
